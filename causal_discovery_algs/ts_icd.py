...
class LearnStructTSICD(LearnStructICD):
    def __init__(self, nodes_sets_list, ci_test, is_homology=True, initial_pag: PAG = None, is_pre_calc_cond_set=False,
        ...
        """
        warnings.warn("TS-ICD is currently under development, bugs may be present", RuntimeWarning)
        ...
        assert any((isinstance(nodes_t, list) and len(nodes_t) == num_nodes_t0)
    ...
    def get_ci_homology_1(self, node_0, node_1, conditioning_tuple) -> list:
        """
        A homology map finds all the node-pairs that are homologous to a given pair.
        Homology of conditioning sets is restricted to the time window length. That is, ignoring conditioning sets
        that are outside the time-window, but still returning the node-pair as independent.
        :param node_0: node at present time
        :param node_1: node in present time (contemporaneous) or in the past (cross-lag)
        ...
        past_time = time_1  # conditioning sets might be further back in time. Those outside the time-window: omitted
        homology = []
        # loop through new time indexes for node_0  and node_i (iteratively move one step back in time)
        for idx, _ in enumerate(range(past_time+1, self.time_window_len)):
            ...
            conditioning_tuple_new = tuple(self._get_node_name(node_time+1+idx, node_idx)
                                           ...
                                           if node_time+1+idx < self.time_window_len)  # only withing the time-window
    ...
    def _learn_struct_incremental_step(self, cond_set_size=None):
        ...
        done1 = self._learn_struct_incremental_for_edges(cond_set_size, self.possible_cross_lag_edges)  # reduce exogenous
        done2 = self._learn_struct_incremental_for_edges(cond_set_size, self.possible_contemporaneous_edges)  # contemporaneous
    ...
    def _learn_struct_incremental_for_edges(self, cond_set_size, possible_edges):
        ...
        for node_i, node_j in possible_edges:
            ...
            for cond in cond_sets:
                ...
                #     if self.is_homology:
                #         homology = self.get_homology(node_0=node_i, node_1=node_j)
